<template>
  <v-container id="residential-admin">
    <v-row>
      <v-col>
        <v-col align="center" justify="center">
          ...
          <v-row align="center" justify="center" class="my-8">
            <v-col
            v-for="(image, i) in residentialGallery"
            :key="i"
            cols="6" md="3">
...
export default {
  ...
  methods: {
    ...mapActions(['removeImage', 'createResidentialPost', 'updateResidentialPost', 'removeResidentialPost', 'addResidentialFilters', 'removeResidentialFilters']),
    exit() {
      if (this.imgRef) {
    ...
    onUpload() {
      this.imgRef = `residentialGallery/${this.imageData.name}_${Math.ceil(Math.random()*100000)}`;
    ...
    add() {
      ...
      if (this.imgRef) {
        this.createResidentialPost(post);
      } else {
    ...
    remove(data) {
      ...
      this.removeResidentialPost(data.id);
      this.update = false;
    },
    removeOnReload() {      
      if (this.imgRef) {
    ...
    addFilter(id) {
      ...
      if (post.data) this.addResidentialFilters(post);
      this.newColor = '';
      this.newRoomType = '';
      this.newStyle = '';
    },
    removeFilter(id, data) {
      ...
      if (post.data) this.removeResidentialFilters(post);
    }
  },
  created() {
